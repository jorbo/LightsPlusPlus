...
//        ASSERT_FALSE(light.deleteLight(bridge)[0].isMember("error"));
//    }
    ...
    TEST(lights_test, getHue){
        ASSERT_EQ(light.getHue(bridge), 46406);
    }

    TEST(lights_test, setSaturation){
        ASSERT_FALSE(light.setSaturation(bridge, 254)[0].isMember("error"));
    }
    TEST(lights_test, getSaturation){
        ASSERT_EQ(light.getSaturation(bridge), 254);
    }
    ...
    TEST(lights_test, getNewLights){
        ASSERT_NO_THROW(Light::getNewLights(bridge)[0].isMember("error"));
    }
}